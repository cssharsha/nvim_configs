...
local function parse_curl_args(self, code_opts)
	return {
		...
		body = vim.tbl_extend("force", {
			...
			options = {
				num_ctx = (self.options and self.options.num_ctx) or 4096, -- Ensure a default value
				temperature = code_opts.temperature or (self.options and self.options.temperature) or 0, -- Ensure a default value
			},
			tools = vim.tbl_isempty(code_opts.tools or {}) and nil or code_opts.tools, -- Optional tool support
